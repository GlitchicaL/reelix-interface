...
function VideoCard({ vaultId, collectionId, video, height = 80, font = "text-2xl" }: CardProps) {
  return (
    <motion.div
    ...
    >
      <Link href={`/vaults/${vaultId}/collections/${collectionId}/videos/${video.id}`}>
        <div className="relative overflow-hidden">
          ...
          />
          <p className={`bg-linear-to-t from-card-500 to-card-500/0 absolute bottom-0 w-full p-2 text-center font-bold pointer-events-none ${font}`}>{video.title}</p>
        </div>
      </Link>
    </motion.div>
  );
